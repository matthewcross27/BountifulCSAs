...
export function DashboardApp() {
  ...
  const [showPublishBanner, setShowPublishBanner] = React.useState(false);
    ...
    <>
        ...
        {showPublishBanner ? (
          <div style={{ marginBottom: "var(--space-5)" }}>
            <Callout tone="good" title="Week 12 is out"
              ...
              action={<Button size="sm" variant="quiet" onClick={() => setShowPublishBanner(false)}>Dismiss</Button>}>
      ...
      <Dialog open={publishing} onClose={() => setPublishing(false)} eyebrow="WEEK 12" title="Publish Tuesday's box?"
        footer={<>
          ...
          <Button onClick={() => { setPublishing(false); setPublished(true); setShowPublishBanner(true); }}>Publish</Button>
        </>}>
